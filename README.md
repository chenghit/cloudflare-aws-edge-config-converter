...
- **Model**: `claude-sonnet-4.6-1m` minimum. Switch with `/model` in Kiro.
  ...
  - **CDN migration**: `claude-sonnet-4.6-1m` regardless of domain count. CDN bottleneck is context window (reading all rule files + references per domain), not output tokens — each domain is processed independently and generates ~200 lines of Terraform, well within Sonnet's 64K output limit. Opus is not needed.
