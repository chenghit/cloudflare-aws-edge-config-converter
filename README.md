...
- **Powers-optimized**: File locations and naming conventions are designed for this tool's workflow
...
- **Unpredictable file organization**: Powers cannot reliably locate configuration files without a standard structure
...
| **cloudflare-to-cloudfront-config-converter** | Cloudflare CDN configuration (Cache, Origin, SSL, etc.) | CloudFront Distribution configuration (Terraform) | 🚧 In Development |
...
**CloudflareBackup's predictable structure** allows Powers to:
1. **Automatically locate files**: Powers know exactly where to find `WAF-Custom-Rules.txt`
2. **Validate completeness**: Powers can check if expected files exist
3. **Handle relationships**: Powers know `List-Items-ip-<name>.txt` corresponds to lists in `IP-Lists.txt`

**cf-terraforming's flexible output** creates problems:
1. **No file discovery**: Powers cannot guess what users named their files
2. **No standard organization**: Users might organize files in any directory structure
3. **Manual coordination required**: Users would need to tell Powers where each file is located
...
Power: [Automatically finds WAF-Custom-Rules.txt, Rate-limits.txt, IP-Lists.txt]
```

**With cf-terraforming** (hypothetical):
```
User: "Convert Cloudflare security rules in ./my_terraform/"
Power: "I cannot find standard configuration files. Please specify:
