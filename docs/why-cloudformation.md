...
## Fitting AWS's hard per-WebACL caps

AWS caps a WebACL at **10 rate-based rules** and **50 reference statements** (IP-set + regex-set + rule-group + managed-rule-group references), both non-adjustable — not even via Service Quotas. Rather than split one WebACL per proxied domain (100 domains → 100 WebACLs), the pipeline uses a **rule-group overflow packer**: overflow rate-based rules and IP-set references are moved into referenced custom rule groups, which escape both caps (a rule group holds ≤4 RBR and ≤50 references, and the WebACL pays just 1 reference for the whole group). Cloudflare phase order (custom → rate → managed) and label semantics are preserved — every `LabelMatchStatement` is rewritten to the correct bare/fully-qualified form when a rule moves across the WebACL↔rule-group boundary. So the default is a stable **2 WebACLs** even for configs that reference far more than 50 IP sets.

`--force-split` (one WebACL per proxied domain, host conditions stripped) remains available if you want it for other reasons; it uses the same packer. The generated deployment README includes a Quota Usage section showing actual reference counts per WebACL. A config is only undeployable when a WebACL's WCU exceeds the 5000 hard cap or a single rule is too complex to fit one rule group — reported as `STATUS: BLOCKED`.
