...
| Per rule group | ≤4 rate-based rules, ≤50 references, ≤5000 WCU | The packer fills a group up to ≤4 RBR / ≤50 refs and a **4500-WCU budget** (5000 cap minus a safety margin), then starts another group. |
...
- **`mitigation_timeout` (block duration) is lost.** Cloudflare can block a client for a fixed period after a breach; AWS WAF only blocks while the trailing-window rate stays over the limit (unblocks ~≤30s after it drops). AWS has no equivalent, so this field is not carried through — the rate rule still converts (limit + window), only the fixed block duration is dropped. This whole-feature limitation is documented here; it is not called out per rule in the generated deployment report.
...
- **Some rate rules are dropped entirely (reported `NON_CONVERTIBLE`).** When a rate rule's counting semantics can't be reproduced by an AWS rate-based rule, the whole rule is reported for manual recreation instead of being converted to a wrong threshold. Three cases: `requests_to_origin` (Cloudflare counts only origin-bound / cache-miss requests, but a CloudFront-scoped WebACL runs before the cache and counts every request, so the same threshold trips far sooner), `counting_expression` (a separate counting expression has no AWS equivalent), and any aggregation whose characteristics are not a subset of `{ip.src, cf.colo.id}` containing `ip.src` (header/cookie/path/ASN keys would need RBR `CUSTOM_KEYS`, and a `cf.colo.id`-only or empty set has no per-source-IP key). Each such rule is listed in the deployment README.
