...
- **Conditional bypass → a viewer-request CloudFront Function forces a guaranteed cache MISS.** For matching requests the CFF injects a header (`x-cf-cache-bypass`) with a per-request-unique value that is part of the cache key, so every matching request gets a unique key → always a miss → always fetched from origin. The value is FOUR `Math.random()` segments joined by `-` (string concatenation preserves each segment's ~52 bits ≈ 208 bits total; adding/multiplying would collapse them back to ≤52 bits and skew the distribution — a collision would serve one user's personalized page to another). The `else` branch DELETES the header, so a client can't self-inject it to poison the shared cache. **Verified live on a real CloudFront distribution**: matching requests always miss with distinct busters; anonymous requests cache normally; a client-injected header is stripped and shares the anonymous cache entry.
- **The buster header is whitelisted in the cache policy of every behavior of a bypass domain.** The viewer-request CFF is shared across a domain's behaviors, so the injected header must be in each behavior's cache key or the miss isn't forced. The header name is a single shared constant (`CACHE_BYPASS_HEADER` in cdn_expr_parser) used by both the CFF codegen and the cache-policy whitelist, so they can't drift (the ORP split-brain lesson).
- **Unconditional bypass → the managed CachingDisabled policy** (all TTLs 0, no cache-key inputs) instead of the old custom `min_ttl=0` policy — no CFF needed when the whole behavior never caches.
- **Bypass conditions are not limited to cookies** (WordPress was just one case). All of these convert: a query string (`http.request.uri.query contains "test=true"` → `_qs(...).includes(...)`), a named query arg (`http.request.uri.args["test"] eq "true"` / existence → `request.querystring['test']`), a named request header (`http.request.headers["x-bypass"]` — header names lowercased to match CFF's `request.headers` keys), user-agent/path substrings, and cookies. Cookie/header/arg named-field access is unified: existence renders as `map['n'] !== undefined`, a scalar value as an existence-guarded `.value` comparison.
- **Cookie conditions.** `http.cookie contains "…"` (the whole Cookie header string) is rebuilt in the CFF from the parsed `request.cookies` map via a `_cookieStr` helper and matched with `.includes()` — faithful to Cloudflare's whole-string semantics (a `contains "foo=bar"` matches across the name=value boundary). `http.request.cookies["name"]` existence renders as `request.cookies['name'] !== undefined`. A subtle bug caught by live testing: the first implementation read `request.headers.cookie`, which does NOT exist in a CloudFront Function (cookies arrive parsed in `request.cookies`, not as a raw header) — it would have silently never matched.
- **The `any(field["k"][*] == "v")` multi-value form is non-convertible, and no longer silently over-bypasses.** It parses to a raw expression (the `any()` wrapper is unsupported); a bypass rule with an unparseable condition is now reported non-convertible instead of being treated as unconditional — which would have set the whole behavior to CachingDisabled and disabled caching for far more than the rule intended.

Verified: both test suites green (new cache-bypass checks covering cookie/querystring/header/arg parsing + rendering, cache-buster codegen, CachingDisabled placement, the any() non-convertible guard, and the header-whitelist single-source-of-truth), full 54-domain example pipeline green through every stage (the real `ext.example.com` has an extension-bypass cache rule that now converts correctly), all generated JS `node --check`s, `terraform validate` on the generated cache policies, and a four-scenario live experiment on a real CloudFront distribution (logged-in cookie → always miss with distinct busters; anonymous → caches; client-injected header → stripped; unrelated cookie → caches).
