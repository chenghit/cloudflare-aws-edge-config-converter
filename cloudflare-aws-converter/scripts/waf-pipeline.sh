...
if [ -n "$REF_EXCEEDED" ]; then
    echo "POST_ACTION: PRINT the following WARNING to the user exactly as-is:"
    echo "  ⚠️ This WebACL references $REF_EXCEEDED IP sets (AWS limit: 50). Deployment will fail with this configuration."
    echo "  Options:"
